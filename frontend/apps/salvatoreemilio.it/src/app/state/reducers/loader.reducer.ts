...
export const LOADER_ADAPTER: EntityAdapter<Loader> = createEntityAdapter<Loader>({
...
export const LOADER_REDUCER = createReducer(INITIAL_LOADER_STATE,
  ...
  on(LOADER_ACTIONS.LOADER_OFF, (state, { key }) => {
    ...
    if (value < 1) {
      return LOADER_ADAPTER.removeOne(key, state.ids.length > 1 ? state : { ...state, loading: false })
    }
    return LOADER_ADAPTER.updateOne({
      id: key,
      changes: {
